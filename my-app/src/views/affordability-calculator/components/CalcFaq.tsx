...
import { affordabilityFaqs } from '@/mocks/calcData';
...
export default function CalcFaq() {
  ...
  useEffect(() => {
    ...
    script.id = 'affordability-faq-schema';
    script.textContent = JSON.stringify({
      '@context': 'https://schema.org',
      '@type': 'FAQPage',
      mainEntity: affordabilityFaqs.map((item) => ({
        '@type': 'Question',
        name: item.q,
    ...
    return () => {
      document.getElementById('affordability-faq-schema')?.remove();
    };
  }, []);
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-16">
          <Reveal className="lg:sticky lg:top-28 lg:self-start">
            ...
            <h2 className="mt-6 font-heading text-[32px] font-normal leading-[1.05] tracking-[-0.02em] text-foreground-950 md:text-[46px]">
              Affordability, <em className="italic">answered</em>
            </h2>
            <p className="mt-6 text-[15.5px] leading-relaxed text-foreground-600 md:text-[16.5px]">
              The questions DMV buyers ask most about financing, down payments and how far their budget reaches.
            </p>
          </Reveal>

          <div className="border-t border-background-300">
            {affordabilityFaqs.map((faq, index) => {
