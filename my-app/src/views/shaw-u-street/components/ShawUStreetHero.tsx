...
import { shawHero } from '@/mocks/shawUStreetData';
import { CtaButton, Eyebrow, GhostButton } from '@/views/home-v2/components/shared';

export default function ShawUStreetHero() {
  return (
    <section id="top" className="relative min-h-[94svh] w-full overflow-hidden bg-foreground-950">
      ...
      <div className="pointer-events-none absolute right-7 top-1/2 hidden -translate-y-1/2 xl:block">
        <span className="v2-vertical text-[10px] font-semibold uppercase tracking-[0.42em] text-background-200/60">
          Area Guide — Shaw / U Street Corridor
        </span>
      </div>
      ...
      <div className="relative mx-auto flex min-h-[94svh] max-w-[1500px] flex-col justify-between gap-12 px-5 pb-14 pt-36 md:px-10 md:pb-20 md:pt-44">
        <div className="flex flex-col gap-11 lg:flex-row lg:items-end lg:justify-between lg:gap-16">
          <div className="v2-rise max-w-3xl">
            <Eyebrow tone="light">{shawHero.eyebrow}</Eyebrow>
            <h1 className="text-scrim mt-7 font-heading text-[46px] font-normal leading-[0.96] tracking-[-0.03em] text-background-50 md:text-[76px] lg:text-[94px]">
              Welcome to <em className="italic text-background-100">Shaw / U Street Corridor</em>
            </h1>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-3">
              <CtaButton href={shawHero.primaryCta.href}>{shawHero.primaryCta.label}</CtaButton>
              <GhostButton href={shawHero.secondaryCta.href} tone="light">
                {shawHero.secondaryCta.label}
              </GhostButton>
            </div>
          </div>
          ...
          >
            ...
            <p className="font-heading text-[30px] leading-none text-background-50">Shaw / U Street</p>
            <p className="text-[13.5px] leading-relaxed text-background-200">{shawHero.areaText}</p>
          </div>
        </div>
