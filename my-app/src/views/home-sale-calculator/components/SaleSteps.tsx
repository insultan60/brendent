...
import { saleSteps } from '@/mocks/homeSaleData';
import { Eyebrow } from '@/views/home-v2/components/shared';

export default function SaleSteps() {
  return (
      ...
      <div className="mx-auto max-w-[1280px]">
        <Reveal className="max-w-2xl">
          ...
          <h2 className="mt-6 font-heading text-[32px] font-normal leading-[1.05] tracking-[-0.02em] text-background-50 md:text-[48px]">
            Four steps from a sale price to <em className="italic text-background-100">your net</em>
          </h2>
          <p className="mt-6 text-[15.5px] leading-relaxed text-background-200 md:text-[16.5px]">
            A net proceeds estimate is only useful when it is built the way settlement actually works. Here is the
            order of operations behind the number.
          </p>
        </Reveal>

        <div className="mt-16 grid grid-cols-1 gap-x-10 gap-y-12 sm:grid-cols-2">
          {saleSteps.map((item, index) => (
