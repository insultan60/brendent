...
import { fallsChurchHero } from '@/mocks/fallsChurchData';
import { CtaButton, Eyebrow, GhostButton } from '@/views/home-v2/components/shared';

export default function FallsChurchHero() {
  return (
    <section id="top" className="relative min-h-[94svh] w-full overflow-hidden bg-foreground-950">
      <img
        src={fallsChurchHero.image}
        alt="The Falls Church, Virginia Broad Street downtown and tree-lined neighborhoods — the area guide market served by BW Metro Properties"
        title="Falls Church, VA Area Guide — BW Metro Properties"
      ...
      <div className="relative mx-auto flex min-h-[94svh] max-w-[1500px] flex-col justify-between gap-12 px-5 pb-14 pt-36 md:px-10 md:pb-20 md:pt-44">
        <div className="flex flex-col gap-11 lg:flex-row lg:items-end lg:justify-between lg:gap-16">
          <div className="v2-rise max-w-3xl">
            <Eyebrow tone="light">{fallsChurchHero.eyebrow}</Eyebrow>
            <h1 className="text-scrim mt-7 font-heading text-[48px] font-normal leading-[0.96] tracking-[-0.03em] text-background-50 md:text-[88px] lg:text-[108px]">
              Welcome to <em className="italic text-background-100">Falls Church</em>
            </h1>
            <p className="mt-7 max-w-2xl text-[15px] leading-relaxed text-background-100 [text-shadow:0_1px_16px_rgba(0,0,0,0.45)] md:text-[17px]">
              {fallsChurchHero.text}
            </p>
            <div className="mt-10 flex flex-wrap items-center gap-3">
              <CtaButton href={fallsChurchHero.primaryCta.href}>{fallsChurchHero.primaryCta.label}</CtaButton>
              <GhostButton href={fallsChurchHero.secondaryCta.href} tone="light">
                {fallsChurchHero.secondaryCta.label}
              </GhostButton>
            </div>
          </div>
          ...
          >
            ...
            <p className="font-heading text-[38px] leading-none text-background-50">Falls Church, VA</p>
            <p className="text-[13.5px] leading-relaxed text-background-200">{fallsChurchHero.areaText}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          ...
          >
            {fallsChurchHero.stats.map((stat) => (
