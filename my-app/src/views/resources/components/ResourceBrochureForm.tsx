...
const SUBMIT_ADDR = 'https://readdy.ai/api/form/dalugndh599pccsgtuo0';
...
export default function ResourceBrochureForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const honeypot = String(data.get('mobile_alt') ?? '').trim();
    if (honeypot) {
      setStatus('success');
      setFormError('');
      form.reset();
  ...
  if (status === 'success') {
    return (
      <div className="blog-fade flex flex-col items-center justify-center gap-4 rounded-[26px] border border-background-300 bg-background-50 px-6 py-14 text-center md:p-10">
        <span className="flex h-14 w-14 items-center justify-center rounded-full bg-primary-100 text-primary-700">
          <Check className="h-7 w-7" aria-hidden="true" />
        </span>
        <p className="font-heading text-[26px] leading-tight text-foreground-950">Check your inbox</p>
        <p className="max-w-sm text-[14.5px] leading-relaxed text-foreground-600">
          Your guide is on its way. Brandon will also follow up personally to answer any questions.
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-[26px] border border-background-300 bg-background-50 p-6 md:p-8">
      <form data-readdy-form onSubmit={handleSubmit} className="flex flex-col gap-3.5" noValidate>
        ...
        <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
          <input id="res-book-first" type="text" name="first_name" required placeholder="First Name *" className={fieldClass} />
          <input id="res-book-last" type="text" name="last_name" required placeholder="Last Name *" className={fieldClass} />
        </div>
        <input id="res-book-email" type="email" name="email" required autoComplete="email" placeholder="Email *" className={fieldClass} />
        <input id="res-book-phone" type="tel" name="phone_number" placeholder="Phone" className={fieldClass} />
