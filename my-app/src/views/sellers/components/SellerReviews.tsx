...
import { sellerReviews, sellerReviewsImage } from '@/mocks/sellersData';
...
export default function SellerReviews() {
  ...
  const go = (direction: number) => {
    setIndex((current) => (current + direction + sellerReviews.length) % sellerReviews.length);
  };
            ...
            <div className="mt-8 flex flex-wrap items-center justify-between gap-5">
              <div className="flex items-center gap-2.5">
                ...
                <div className="ml-2 flex items-center gap-2" role="tablist" aria-label="Choose a review">
                  {sellerReviews.map((item, dotIndex) => (
                    <button
                      key={item.name}
                      type="button"
                      role="tab"
