...
const FORM_ENDPOINT = 'https://readdy.ai/api/form/daklagh2o72dhfjvq6c0';
...
/** Source "Sell your home" lead form — captures the property address. */
export default function SellLeadForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!String(data.get('address') ?? '').trim()) {
      setStatus('error');
      setFormError('Please enter your property address so we can get started.');
      return;
    }
      ...
      <div className="flex max-w-2xl items-start gap-4 rounded-[22px] border border-background-50/15 bg-background-50/95 p-6 backdrop-blur-xl">
        ...
        <div>
          ...
          <p className="mt-2 max-w-md text-[14px] leading-relaxed text-foreground-600">
            Brandon will follow up with a tailored plan and an accurate value for your home. Prefer to talk it
            through right now?
          </p>
          <a
            href="#contact"
  ...
  return (
    <div className="w-full">
      ...
      >
        <div className="flex flex-col gap-2 rounded-[22px] border border-background-50/15 bg-background-50/95 p-2.5 backdrop-blur-xl sm:flex-row sm:items-center sm:rounded-full sm:p-2">
          <label className="flex flex-1 items-center gap-3 rounded-2xl px-4 py-3 sm:rounded-full">
            ...
            <span className="sr-only">{sellHero.addressLabel}</span>
            <input
              type="text"
              name="address"
              maxLength={120}
              autoComplete="one-time-code"
              placeholder={sellHero.addressPlaceholder}
          ...
          >
            {status === 'submitting' ? 'Sending\u2026' : sellHero.cta}
          </button>
        </div>
      ...
      <p className="mt-4 max-w-md text-[12.5px] leading-relaxed text-background-200/70">
        No obligation &mdash; get an accurate value and a clear plan before you list.
      </p>
    </div>
  );
}