...
const FORM_ENDPOINT = 'https://readdy.ai/api/form/dakose92o72dhfjvq9o0';
...
/** Source "Cash Offer" lead form — captures the home address. */
export default function CashOfferLeadForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!String(data.get('address') ?? '').trim()) {
      setStatus('error');
      setFormError('Please enter your home address so we can prepare your offer.');
      return;
    }
      ...
      <div className="flex max-w-2xl items-start gap-4 rounded-[22px] border border-background-50/15 bg-background-50/95 p-6 backdrop-blur-xl">
        ...
        <div>
          <p className="font-heading text-[20px] leading-tight text-foreground-950">
            Thanks &mdash; request received.
          </p>
          <p className="mt-2 max-w-md text-[14px] leading-relaxed text-foreground-600">
            Our team is preparing your all-cash offer for the address you shared. Prefer to talk it through right
            now?
          </p>
          <a
            href="#contact"
  ...
  return (
    <div className="w-full">
      ...
      >
        <div className="flex flex-col gap-2 rounded-[22px] border border-background-50/15 bg-background-50/95 p-2.5 backdrop-blur-xl sm:flex-row sm:items-center sm:rounded-full sm:p-2">
          <label className="flex flex-1 items-center gap-3 rounded-2xl px-4 py-3 sm:rounded-full">
            ...
            <span className="sr-only">{cashOfferHero.addressLabel}</span>
            <input
              type="text"
              name="address"
              maxLength={120}
              autoComplete="one-time-code"
              placeholder={cashOfferHero.addressPlaceholder}
          ...
          >
            {status === 'submitting' ? 'Sending\u2026' : cashOfferHero.cta}
          </button>
        </div>
      ...
      <p className="mt-4 max-w-md text-[12.5px] leading-relaxed text-background-200/70">
        No obligation &mdash; a fair, all-cash offer with zero pressure to accept.
      </p>
    </div>
  );
}